...
class PDFDebugLogger:
    """Enhanced logger for PDF processing debugging."""
    
    def __init__(self, name: str = "pdf_debug", level: str = "DEBUG"):
        self.logger = get_logger(name)
        self.logger.setLevel(getattr(logging, level.upper()))
        self._operation_stack: List[str] = []
        self._performance_data: Dict[str, List[float]] = {}
        
    def debug_stage(self, stage_name: str, **kwargs) -> None:
    ...
    def debug_success(self, stage_name: str, **kwargs) -> None:
    ...
    def debug_error(self, stage_name: str, error: Exception, **kwargs) -> None:
    ...
    def debug_data(self, data_name: str, data: Any, preview_length: int = 100) -> None:
        ...
        if isinstance(data, (str, bytes)):
            ...
            if len(str(data)) > preview_length:
                preview += "..."
            self.logger.debug(f"{indent}📊 {data_name}: {type(data).__name__}({len(data)}) = '{preview}'")
        
        elif isinstance(data, (list, tuple)):
            self.logger.debug(f"{indent}📊 {data_name}: {type(data).__name__}({len(data)}) items")
            for i, item in enumerate(data[:3]):  # Show first 3 items
                self.logger.debug(f"{indent}   [{i}]: {type(item).__name__} = {str(item)[:50]}...")
            if len(data) > 3:
                self.logger.debug(f"{indent}   ... and {len(data) - 3} more items")
        
        elif isinstance(data, dict):
            self.logger.debug(f"{indent}📊 {data_name}: {type(data).__name__}({len(data)}) keys")
            for i, (key, value) in enumerate(list(data.items())[:3]):
                self.logger.debug(f"{indent}   {key}: {type(value).__name__} = {str(value)[:50]}...")
            if len(data) > 3:
                self.logger.debug(f"{indent}   ... and {len(data) - 3} more keys")
        
        else:
            self.logger.debug(f"{indent}📊 {data_name}: {type(data).__name__} = {str(data)[:preview_length]}")
    
    def debug_performance(self, operation_name: str, duration: float, **metrics) -> None:
    ...
    def debug_image_processing(self, image_index: int, xref: int, width: int, height: int, 
                             format_type: str, size_bytes: int) -> None:
    ...
    def debug_table_processing(self, table_index: int, rows: int, cols: int, 
    ...
    def debug_equation_processing(self, eq_index: int, eq_type: str, latex_code: str) -> None:
    ...
    def debug_chunking(self, chunk_index: int, chunk_size: int, chunk_type: str, 
                      overlap_size: int = 0) -> None:
    ...
    @contextmanager
    def debug_operation(self, operation_name: str, **context) -> Generator[None, None, None]:
    ...
    def save_performance_report(self, output_path: Union[str, Path]) -> None:
...
_debug_logger: Optional[PDFDebugLogger] = None
...
def get_debug_logger() -> PDFDebugLogger:
    ...
    if _debug_logger is None:
        _debug_logger = PDFDebugLogger()
    return _debug_logger


def init_debug_logging(config: DebugConfig) -> PDFDebugLogger:
    """Initialize debug logging with configuration."""
    global _debug_logger
    
    if config.enabled:
        _debug_logger = PDFDebugLogger(level=config.level)
        _debug_logger.logger.info("Debug logging initialized")
    else:
        # Create a no-op logger
        _debug_logger = PDFDebugLogger(level="CRITICAL")
